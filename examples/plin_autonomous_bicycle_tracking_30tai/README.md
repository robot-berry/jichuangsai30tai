...
- 目标可见时自动小幅转向居中
- 转向居中时同步修正前后距离，不再等待完全居中后才定距
- 连续丢失后按最后目标方向搜索，并周期性反向扫描
- 目标重新出现后立即退出搜索并恢复小幅追踪
- 单目距离估计、稳定滤波，以及 `±0.03 m` 停车、超过 `±0.08 m` 才恢复的定距滞回
- 差速控制：前进 `(+,+)`、后退 `(-,-)`、左转 `(-,+)`、右转 `(+,-)`
...
默认部署完成后即可追踪，不需要再输入开启 CAN 的命令。前后跟随上限为 `45 rpm`，转向上限为 `50 rpm`；云台、红外和丢失搜索保持关闭，目标日志或 CAN 反馈中断时自动归零。只需要无动作预览时增加 `-PreviewOnly`：
...
- 前后跟随为 `45 rpm`，转向命令硬限制为 `50 rpm`，丢失搜索仍限制为 `40 rpm`。
