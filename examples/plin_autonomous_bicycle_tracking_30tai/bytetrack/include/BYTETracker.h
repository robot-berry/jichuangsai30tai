...
public:
	BYTETracker(int frame_rate = 30, int track_buffer = 30,
		float track_thresh_value = 0.30f, float high_thresh_value = 0.45f,
		float match_thresh_value = 0.80f);
	~BYTETracker();

	vector<STrack> update(const vector<byteTracker::Object>& objects);
...
private:
	...
	vector<vector<float> > fuse_score(vector<std::vector<float>>& cost_matrix, vector<STrack>& detections);
