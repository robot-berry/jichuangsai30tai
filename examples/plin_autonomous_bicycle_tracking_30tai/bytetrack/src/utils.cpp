...
vector<vector<float> > BYTETracker::fuse_score(vector<std::vector<float>> &cost_matrix, vector<STrack>& detections) {
	...
	std::vector<std::vector<float>> iou_sim(num_tracks, std::vector<float>(num_detections));
	for (size_t i = 0; i < num_tracks; ++i) {
		for (size_t j = 0; j < num_detections; ++j) {
			iou_sim[i][j] = 1.0f - cost_matrix[i][j];
		}
	}
	...
	std::vector<std::vector<float>> det_scores_expanded(num_tracks, det_scores);

	std::vector<std::vector<float>> fuse_sim(num_tracks, std::vector<float>(num_detections));
	for (size_t i = 0; i < num_tracks; ++i) {
		for (size_t j = 0; j < num_detections; ++j) {
			fuse_sim[i][j] = iou_sim[i][j] * det_scores_expanded[i][j];
		}
	}
	for (size_t i = 0; i < num_tracks; ++i) {
		for (size_t j = 0; j < num_detections; ++j) {
			{
				cost_matrix[i][j] = 1.0f - fuse_sim[i][j];
			}
		}
	}
	return cost_matrix;
	...
	bool extend_cost, float cost_limit, bool return_cost)
{
	...
	else
	{
		if (!extend_cost)
		{
			throw std::invalid_argument("lapjv requires extend_cost for a non-square matrix");
		}
	}
	...
	vector<vector<double> > cost_storage(n, vector<double>(n));
	vector<double *> cost_ptr(n);
	for (int i = 0; i < n; i++)
		cost_ptr[i] = cost_storage[i].data();
	...
	if (ret != 0)
	{
		throw std::runtime_error("lapjv assignment failed");
	}
