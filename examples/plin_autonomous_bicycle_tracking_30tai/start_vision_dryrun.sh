...
case "$DRYRUN_GIMBAL_ENABLE:$DRYRUN_CHASSIS_ENABLE:$DRYRUN_SEARCH_ENABLE" in
    [01]:[01]:[01]) ;;
    *)
...
(cat "$LOG_DIR/plin_stdin.fifo" | timeout "$RUN_SECONDS" env \
    ...
    AIM_FOLLOW_MIN_FOLLOW_RPM=50 \
    AIM_FOLLOW_MAX_FOLLOW_RPM=50 \
    AIM_FOLLOW_CHASSIS_STEER_ENABLE=1 \
    AIM_FOLLOW_STEER_DEADZONE_NORM=0.08 \
    AIM_FOLLOW_STEER_KP_RPM=45 \
    AIM_FOLLOW_MIN_STEER_RPM=60 \
    AIM_FOLLOW_MAX_STEER_RPM=60 \
...
echo "[VISION READY] pid=$APP_PID can_dryrun=1 gimbal=$DRYRUN_GIMBAL_ENABLE chassis=$DRYRUN_CHASSIS_ENABLE search=$DRYRUN_SEARCH_ENABLE synthetic=$SYNTHETIC_TARGET"
